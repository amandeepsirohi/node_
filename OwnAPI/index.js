...
app.get("/filter", (req, res) => {
    const aa = req.query.type;
    const foundJoke = jokes.filter((joke) => joke.jokeType === aa);
    res.json(foundJoke);
});
...
app.put("/jokes/:id", (req, res) => {
    ...
    const index = jokes.findIndex((jokes) => jokes.id === id);
    jokes[index] = oo;
    res.json(oo);
});
//6. PATCH a joke
...
app.delete("/jokes/:id", (req, res) => {
    ...
    if (searchIndex > -1) {
        ...
        res.status(404).json({ error: "No Jokes were deleted because id doesn't exist" });
...
app.delete("/all", (req, res) => {
    ...
    if (userKey === masterKey) {
        ...
        res.send(404).json({ error: "Wrong key" });
    }
});
app.listen(port, () => {
